# Attempting to determine if the reports are safe or not
# conditions are all numbers are increasing or decreasing and 
# that two levels differ by at least 1 and at most 3
#
# can create a data structure which contains the following attributes:
#
# data: list of ints
# safety: string stating if report is safe or unsafe
# direction: string stating if integers are increasing or decreasing
#
# can also write methods that initialize the attributes and test for the conditions
...
#move for report in inputList safety testing into a function so i can test each report individually
#it'll allow me to retest with the spliced reports as well to determine if omitting a level will make
#it safe

def safeReport(report: list) -> bool:
    #report is the list of numbers and i can pass this to other functions to test safety
    diff_pos = set([1,2,3])
    diff_neg = set([-1,-2,-3])
    ...
    for i in range(1,len(report)):
        ...
        diff_pos.add(diff)
        diff_neg.add(diff)
    if len(diff_pos) == 3 or len(diff_neg) == 3:
        return True
    return False
    ...
    for report in inputList:
        if safeReport(report):
            ...
            for i in range(len(report)):
                omitted_report = report[:i] + report[i+1:]
                if safeReport(omitted_report):
                    safe_count += 1
                    break
    return safe_count
